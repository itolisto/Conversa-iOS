...
- (void)refreshNavigationBarInformation:(YapContact *)buddy {
    __weak typeof(self) wSelf = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        UIImage *image = [[NSFileManager defaultManager] loadAvatarFromLibrary:[buddy.uniqueId stringByAppendingString:@"_avatar.jpg"]];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (sSelf) {
                if (image) {
                    ...
                    [self.avatarView
                     sd_setImageWithURL:[NSURL URLWithString:buddy.avatarThumbFileId]
                     placeholderImage:[UIImage imageNamed:@"ic_business_default"]];
                }
            }
        });
